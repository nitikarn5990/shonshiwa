...
class Model_global_core extends CI_Model{
    ...
    public function _get($config){
        ...
        if(isset($config['join_left'])){
            if(is_array($config['join_left'])){
                foreach($config['join_left'] AS $key => $value){
        ...
        if(isset($config['grouping'])){
            ...
            if(isset($config['grouping']['where'])){
                $this->db->where($config['grouping']['where']);
            }

            if(isset($config['grouping']['like'])) {
                $this->db->like($config['grouping']['like']);
            }

            if(isset($config['grouping']['like_none'])) {
                foreach($config['grouping']['like_none'] AS $k => $d){
                    $this->db->like($k,$d,'none');
                }
            }

            if(isset($config['grouping']['like_before'])) {
                foreach($config['grouping']['like_before'] AS $k => $d){
                    $this->db->like($k,$d,'before');
                }
            }

            if(isset($config['grouping']['or_like'])) {
                $this->db->or_like($config['grouping']['or_like']);
            }
...
//            ON DUPLICATE KEY UPDATE ci_view = ci_view + 1
    ...
    public function _showquery($config=FALSE,$data=array()){
        ...
        if($config==TRUE){
            $error = $this->db->error();
            echo "<p>".$this->db->last_query()."</br>".print_r($data)."</p>".print_r($error);
        }
